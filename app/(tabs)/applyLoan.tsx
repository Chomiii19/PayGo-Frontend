  ...
  const onsSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://paygo-backend-1y0p.onrender.com/api/v1/app/apply-loan",
        {
          amount,
          paymentSource: selectedPaymentSource.toLowerCase(),
        },
    ...
    <View className="flex-1 flex-col bg-backgroundColor justify-between">
      <View className="flex flex-col w-full">
        ...
        <View className="flex flex-col gap-2 px-5 w-full">
          ...
          <Text className="text-zinc-300 font-rSemibold mt-3">Loan Amount</Text>
          <TextInput
          ...
          <View className="mt-1 flex flex-row w-full justify-center">
            <Text className="text-zinc-500 font-rRegular text-xs">
              Be informed that there is a 0.03% interest fee.
            </Text>
          </View>
        </View>
      </View>
